...
void CenterContainer::arrange(){
    for (int i = 0; i < elements.size(); i++){
        (*elements[i]).setPosition(sf::Vector2f((width-(*elements[i]).getSize().x)/2, (height-(*elements[i]).getSize().y)/2));
        sf::Vector2f initPos = (*elements[i]).getPosition();
        (*elements[i]).setPosition(sf::Vector2f(initPos.x, initPos.y + (*elements[i]).marginTop));
